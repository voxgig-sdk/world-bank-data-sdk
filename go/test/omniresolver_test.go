...
import (
	...
	omni "github.com/voxgig-sdk/world-bank-data-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.WorldBankDataError); is {
				out := map[string]any{
					"name":    "WorldBankDataError",
...
func omniCtx(arg any, client *sdk.WorldBankDataSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.WorldBankDataSDK); is {
			client = live
