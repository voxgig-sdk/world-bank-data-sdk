...
func TestMetadataEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set WORLD_BANK_DATA_TEST_METADATA_ENTID JSON to run live")
			return
		}
		client := setup.client

		// Bootstrap entity data from existing test data (no create step in flow).
		metadataRef01DataRaw := vs.Items(core.ToMapAny(vs.GetPath(setup.data, "existing.metadata")))
...
func metadataBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("WORLD_BANK_DATA_TEST_METADATA_ENTID")
	...
	idmapResolved := core.ToMapAny(env["WORLD_BANK_DATA_TEST_METADATA_ENTID"])
