...
func TestTopicEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		topicRef01DataRaw := vs.Items(core.ToMapAny(vs.GetPath(setup.data, "existing.topic")))
		var topicRef01Data map[string]any
		if len(topicRef01DataRaw) > 0 {
			topicRef01Data = core.ToMapAny(topicRef01DataRaw[0][1])
		}
		...
		_, topicRef01ListOk := topicRef01ListResult.([]any)
		if !topicRef01ListOk {
...
func topicBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("WORLD_BANK_DATA_TEST_TOPIC_ENTID")
	...
	idmapResolved := core.ToMapAny(env["WORLD_BANK_DATA_TEST_TOPIC_ENTID"])
