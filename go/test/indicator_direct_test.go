...
func TestIndicatorDirect(t *testing.T) {
	t.Run("direct-list-indicator", func(t *testing.T) {
		setup := indicatorDirectSetup([]any{
		...
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-list-indicator", _mode); _shouldSkip {
			if _reason == "" {
	...
	t.Run("direct-load-indicator", func(t *testing.T) {
		setup := indicatorDirectSetup(map[string]any{"id": "direct01"})
		_mode := "unit"
		if setup.live {
			_mode = "live"
		}
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-load-indicator", _mode); _shouldSkip {
			if _reason == "" {
				_reason = "skipped via sdk-test-control.json"
			}
			t.Skip(_reason)
...
type indicatorDirectSetupResult struct {
...
func indicatorDirectSetup(mockres any) *indicatorDirectSetupResult {
	loadEnvLocal()

	calls := &[]map[string]any{}

	env := envOverride(map[string]any{
		"WORLD_BANK_DATA_TEST_INDICATOR_ENTID": map[string]any{},
	...
	if live {
		...
		idmap := map[string]any{}
		if entidRaw, ok := env["WORLD_BANK_DATA_TEST_INDICATOR_ENTID"]; ok {
		...
		return &indicatorDirectSetupResult{client: client, calls: calls, live: true, idmap: idmap}
	}
	...
	return &indicatorDirectSetupResult{client: client, calls: calls, live: false, idmap: map[string]any{}}
}
