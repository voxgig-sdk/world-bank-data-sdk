// Command WorldBankData-cli is a CLI + REPL for the WorldBankData SDK,
...
const prompt = "world-bank-data"

// entitiesHelp is the space-separated entity list shown by /help.
const entitiesHelp = "country indicator metadata topic"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      WORLD_BANK_DATA_APIKEY (api key), WORLD_BANK_DATA_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: WORLD_BANK_DATA_APIKEY carries the API key and
	// WORLD_BANK_DATA_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("WORLD_BANK_DATA_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("WORLD_BANK_DATA_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewWorldBankDataSDK(opts)
