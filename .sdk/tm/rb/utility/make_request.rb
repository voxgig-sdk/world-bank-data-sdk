...
module WorldBankDataUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = WorldBankDataResponse.new({})
    ...
    if fetch_err
      ...
      response = WorldBankDataResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = WorldBankDataResponse.new(fetched)
    else
