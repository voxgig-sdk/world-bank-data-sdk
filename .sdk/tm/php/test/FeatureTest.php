...
// WorldBankData SDK feature test
//
...
class FtCtrl extends WorldBankDataControl
{
...
class FtClient
{
    ...
    private WorldBankDataUtility $utility;

    public function __construct(WorldBankDataUtility $utility, array $options)
    {
    ...
    public function get_utility(): WorldBankDataUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (WorldBankDataContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public WorldBankDataContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (WorldBankDataContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = WorldBankDataConfig::make_feature($name);
    ...
    private static function buildUrl(WorldBankDataSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof WorldBankDataError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new WorldBankDataResponse($fetched);
        }
    ...
    private function fail(WorldBankDataContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (WorldBankDataContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
