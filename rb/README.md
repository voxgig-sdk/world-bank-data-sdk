...
The Ruby SDK for the WorldBankData API — an entity-oriented client using idiomatic Ruby conventions.
...
```ruby
result, err = client.Country().load({ "id" => "example_id" })
raise err if err
puts result
```
...
result, err = client.WorldBankData().load({ "id" => "test01" })
