...
class TopicDirectTest < Minitest::Test
  def test_direct_list_topic
    setup = topic_direct_setup([
      { "id" => "direct01" },
      { "id" => "direct02" },
    ])
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-list-topic", setup[:live] ? "live" : "unit")
    if _should_skip
      skip(_reason || "skipped via sdk-test-control.json")
      return
    end
    if setup[:live]
      ["topic01"].each do |_live_key|
...
def topic_direct_setup(mockres)
