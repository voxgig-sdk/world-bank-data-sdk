...
class CountryDirectTest < Minitest::Test
  def test_direct_list_country
    setup = country_direct_setup([
      { "id" => "direct01" },
      { "id" => "direct02" },
    ])
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-list-country", setup[:live] ? "live" : "unit")
    if _should_skip
  ...
  def test_direct_load_country
    setup = country_direct_setup({ "id" => "direct01" })
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-load-country", setup[:live] ? "live" : "unit")
    if _should_skip
      skip(_reason || "skipped via sdk-test-control.json")
      return
    end
...
def country_direct_setup(mockres)
