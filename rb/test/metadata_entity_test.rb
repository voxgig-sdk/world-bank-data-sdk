...
class MetadataEntityTest < Minitest::Test
  ...
  def test_basic_flow
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set WORLD_BANK_DATA_TEST_METADATA_ENTID JSON to run live"
      return
    end
    client = setup[:client]
    ...
    metadata_ref01_data = nil
    if metadata_ref01_data_raw.length > 0
...
def metadata_basic_setup(extra)
  ...
  entid_env_raw = ENV["WORLD_BANK_DATA_TEST_METADATA_ENTID"]
