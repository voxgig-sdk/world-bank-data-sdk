...
class TopicEntityTest < Minitest::Test
  ...
  def test_basic_flow
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set WORLD_BANK_DATA_TEST_TOPIC_ENTID JSON to run live"
      return
    end
    client = setup[:client]
    ...
    topic_ref01_data = nil
    if topic_ref01_data_raw.length > 0
...
def topic_basic_setup(extra)
  ...
  entid_env_raw = ENV["WORLD_BANK_DATA_TEST_TOPIC_ENTID"]
