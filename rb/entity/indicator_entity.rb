...
class IndicatorEntity
  ...
  def make
    opts = @_entopts.dup
    IndicatorEntity.new(@_client, opts)
  end
  ...
  #   as an empty match, so client.Indicator.load works with no args).
  ...
  #
  # @param reqmatch [IndicatorListMatch, Hash, nil] match filter (any subset of
  #   Indicator fields); defaults to nil, treated as an empty match that lists all.
  # @param ctrl [Object, nil] optional per-call control
