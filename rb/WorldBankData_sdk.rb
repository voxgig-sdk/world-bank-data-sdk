...
require_relative 'WorldBankData_types'
...
class WorldBankDataSDK
  ...
  def initialize(options = {})
    ...
    feature_opts = WorldBankDataHelpers.to_map(VoxgigStruct.getprop(@options, "feature"))
    if feature_opts
      featureorder = VoxgigStruct.getpath(@options, "__derived__.featureorder")
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = WorldBankDataHelpers.to_map(feature_opts[fname])
          if fopts && fopts["active"] == true
            utility.feature_add.call(@_rootctx, WorldBankDataFeatures.make_feature(fname))
          end
        end
      end
    end
  ...
  def get_utility
    WorldBankDataUtility.copy(@_utility)
  end
  ...
  def prepare(fetchargs = {})
    ...
    ctrl = WorldBankDataHelpers.to_map(VoxgigStruct.getprop(fetchargs, "ctrl")) || {}
    ...
    params = WorldBankDataHelpers.to_map(VoxgigStruct.getprop(fetchargs, "params")) || {}
    query = WorldBankDataHelpers.to_map(VoxgigStruct.getprop(fetchargs, "query")) || {}
  ...
  def raw_request(fetchargs = {})
    ...
    if fetched.is_a?(Hash)
      status = WorldBankDataHelpers.to_int(VoxgigStruct.getprop(fetched, "status"))
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = WorldBankDataError.new(
        "graphql_error", "WorldBankDataSDK: graphql: #{msg}")
      res["graphql"] = errors
    end
  ...
  def self.test(testopts = nil, sdkopts = nil)
    ...
    sdk = WorldBankDataSDK.new(sdkopts)
    sdk.mode = "test"
    sdk
  end
end
