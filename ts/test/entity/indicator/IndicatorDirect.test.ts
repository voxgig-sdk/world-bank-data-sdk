...
describe('IndicatorDirect', async () => {
  ...
  test('direct-load-indicator', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-indicator', setup.live)) return
    if (skipIfMissingIds(t, setup, ["country_code01"])) return
  ...
  test('direct-list-indicator', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }])
    if (maybeSkipControl(t, 'direct', 'direct-list-indicator', setup.live)) return
    const { client, calls } = setup
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['WORLD_BANK_DATA_TEST_INDICATOR_ENTID']
