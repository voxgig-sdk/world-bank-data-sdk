...
describe('TopicDirect', async () => {
  ...
  test('direct-list-topic', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }])
    if (maybeSkipControl(t, 'direct', 'direct-list-topic', setup.live)) return
    if (skipIfMissingIds(t, setup, ["topic01"])) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['WORLD_BANK_DATA_TEST_TOPIC_ENTID']
