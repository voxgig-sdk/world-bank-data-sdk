...
//     (\`new WorldBankDataSDK(...)\` / \`WorldBankDataSDK.test(...)\` become
//     \`WorldBankDataSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const SDK_NAME = 'WorldBankDataSDK'
...
const TEST_SEED = {"entity":{"country":{"test01":{"id":"test01"}},"indicator":{"test01":{"id":"test01"}},"metadata":{"test01":{"id":"test01"}},"topic":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new WorldBankDataSDK(...)\` and any
// \`WorldBankDataSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+WorldBankDataSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/WorldBankDataSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
