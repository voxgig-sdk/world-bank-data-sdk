...
// WorldBankData SDK retry feature
...
class WorldBankDataRetryFeature extends WorldBankDataBaseFeature
{
    ...
    public function init(WorldBankDataContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (WorldBankDataContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(WorldBankDataContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(WorldBankDataContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
