...
The PHP SDK for the WorldBankData API. Provides an entity-oriented interface using PHP conventions.
...
```php
[$result, $err] = $client->Country(null)->list(null, null);
...
```php
[$result, $err] = $client->Country(null)->load(["id" => "example_id"], null);
if ($err) { throw new \Exception($err); }
print_r($result);
```
...
[$result, $err] = $client->WorldBankData(null)->load(
    ["id" => "test01"], null
);
