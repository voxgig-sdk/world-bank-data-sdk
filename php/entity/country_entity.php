...
class CountryEntity
{
    ...
    {
        $opts = $this->_entopts;
        return new CountryEntity($this->_client, $opts);
    }

    /**
     * @param Country|array $args Country data (assoc-array) to store.
     */
    public function data_set($args): void
    {
        if ($args) {
            $this->_data = WorldBankDataHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetData");
        }
    }

    /**
     * @return Country|array The current Country data as an assoc-array.
     */
    ...
    /**
     * @param array $args Match filter (any subset of Country fields).
     */
    public function match_set($args): void
    {
        if ($args) {
            $this->_match = WorldBankDataHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetMatch");
        }
    }

    /**
     * @return array The current match filter (any subset of Country fields).
     */
     ...
     *
     * @param CountryLoadMatch|array|null $reqmatch Match criteria (id/query
     *   fields) as an assoc-array; a typed CountryLoadMatch names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return Country|array The loaded Country as an assoc-array at the
        ...
        return $this->_run_op($ctx, function () use ($ctx) {
            if ($ctx->result) {
                ...
                if ($ctx->result->resdata) {
                    $this->_data = WorldBankDataHelpers::to_map(Struct::clone($ctx->result->resdata)) ?? [];
                }
            }
        });
    }
     ...
     *
     * @param CountryListMatch|array|null $reqmatch Match filter (any subset
     *   of Country fields) as an assoc-array; CountryListMatch names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return Country[]|array A list of Country items as assoc-arrays at
