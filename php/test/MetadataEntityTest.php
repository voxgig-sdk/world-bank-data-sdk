...
class MetadataEntityTest extends TestCase
{
    public function test_create_instance(): void
    {
        $testsdk = WorldBankDataSDK::test(null, null);
        $ent = $testsdk->Metadata(null);
    ...
    public function test_stream(): void
    {
        ...
        $base = WorldBankDataSDK::test($seed, null);
        $seen = iterator_to_array($base->Metadata(null)->stream("list", null, null), false);
        $this->assertCount(3, $seen);

        // Inbound: streaming active -> yields each item from the feature.
        $cfg = WorldBankDataConfig::shared_config();
        if (isset($cfg["feature"]) && is_array($cfg["feature"]) && isset($cfg["feature"]["streaming"])) {
            $sdk = WorldBankDataSDK::test($seed, ["feature" => ["streaming" => ["active" => true]]]);
            $got = [];
            foreach ($sdk->Metadata(null)->stream("list", null, null) as $item) {
    ...
    public function test_basic_flow(): void
    {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set WORLD_BANK_DATA_TEST_METADATA_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
        ...
        $metadata_ref01_data = null;
        if (count($metadata_ref01_data_raw) > 0) {
...
function metadata_basic_setup($extra)
{
    ...
    $entity_data_file = __DIR__ . '/../../.sdk/test/entity/metadata/MetadataTestData.json';
    ...
    $entid_env_raw = getenv("WORLD_BANK_DATA_TEST_METADATA_ENTID");
    ...
    $idmap_resolved = Helpers::to_map(
        $env["WORLD_BANK_DATA_TEST_METADATA_ENTID"]);
