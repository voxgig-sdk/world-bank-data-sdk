...
class TestIndicatorDirect:

    def test_should_direct_list_indicator(self):
        setup = _indicator_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-indicator", "live" if setup["live"] else "unit")
        if _skip:
    ...
    def test_should_direct_load_indicator(self):
        setup = _indicator_direct_setup({"id": "direct01"})
        _skip, _reason = runner.is_control_skipped("direct", "direct-load-indicator", "live" if setup["live"] else "unit")
        if _skip:
            # pytest already imported at module scope
            pytest.skip(_reason or "skipped via sdk-test-control.json")
            return
...
def _indicator_direct_setup(mockres):
