...
class WorldBankDataTestRunner:
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            WorldBankDataTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = WorldBankDataTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = WorldBankDataTestRunner.getenv("WORLD_BANK_DATA_TEST_LIVE")
        override = WorldBankDataTestRunner.getenv("WORLD_BANK_DATA_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = WorldBankDataTestRunner.getenv(key)
        ...
        explain = WorldBankDataTestRunner.getenv("WORLD_BANK_DATA_TEST_EXPLAIN")
    ...
    def load_test_control():
        ...
        """
        if WorldBankDataTestRunner._test_control is not None:
            return WorldBankDataTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                WorldBankDataTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            WorldBankDataTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = WorldBankDataTestRunner.load_test_control()
...
def load_env_local():
    WorldBankDataTestRunner.load_env_local()


def env_override(m):
    return WorldBankDataTestRunner.env_override(m)


def entity_data(v):
    return WorldBankDataTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return WorldBankDataTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return WorldBankDataTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return WorldBankDataTestRunner.load_test_control()


def live_client_options():
    return WorldBankDataTestRunner.live_client_options()


def live_delay_ms():
    return WorldBankDataTestRunner.live_delay_ms()
