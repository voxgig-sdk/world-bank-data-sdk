...
[MCP](https://modelcontextprotocol.io) server exposing the WorldBankData SDK as
two agent tools — `world-bank-data_list` and `world-bank-data_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/world-bank-data-mcp)
make build
...
claude mcp add --scope user world-bank-data \
  -- /absolute/path/to/world-bank-data-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./world-bank-data-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// world-bank-data_list: first page of records
{ "entity": "country" }
{ "entity": "country", "query": { } }

// world-bank-data_load: one record by id
{ "entity": "country", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/world-bank-data-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user world-bank-data \
     -- "$PWD"/dist/*/world-bank-data-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `world-bank-data_list` and `world-bank-data_load` tools now appear
   in new sessions. Ask the agent to *"list country using world-bank-data"*
   and it calls `world-bank-data_list` with `{"entity":"country"}`.
...
### Call the `world-bank-data_list` tool
...
### Call the `world-bank-data_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `world-bank-data_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `world-bank-data_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
```sh
./world-bank-data-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"world-bank-data_load","arguments":{"entity":"country","query":{"id":1}}}}'
```
