...
type Args struct {
	Entity string         `json:"entity" jsonschema:"country | indicator | metadata | topic"`
	Query  map[string]any `json:"query,omitempty" jsonschema:"optional match map e.g. {\"id\":1} for load, omit for list"`
}

func registerTools(server *mcp.Server, client *sdk.WorldBankDataSDK) {
	mcp.AddTool(server, &mcp.Tool{
		Name: "world-bank-data_list",
		Description: "List records from WorldBankData. " +
	...
	mcp.AddTool(server, &mcp.Tool{
		Name: "world-bank-data_load",
		Description: "Load a single record from WorldBankData. " +
...
func runOp(client *sdk.WorldBankDataSDK, op string, args Args) (*mcp.CallToolResult, any, error) {
...
func entityFor(client *sdk.WorldBankDataSDK, name string) (sdk.WorldBankDataEntity, error) {
	switch strings.ToLower(name) {
	case "country":
		return client.Country(nil), nil
	case "indicator":
		return client.Indicator(nil), nil
	case "metadata":
