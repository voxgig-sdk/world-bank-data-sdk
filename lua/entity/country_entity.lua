...
local CountryEntity = {}
CountryEntity.__index = CountryEntity


function CountryEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, CountryEntity)
  self._name = "country"
  self._client = client
...
function CountryEntity:get_name()
  return self._name
end


function CountryEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return CountryEntity.new(self._client, opts)
end
...
function CountryEntity:mark_deleted()
  self._deleted = true
end


function CountryEntity:deleted()
  return true == self._deleted
end


function CountryEntity:data_set(args)
  if args ~= nil then
...
function CountryEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function CountryEntity:match_set(args)
  if args ~= nil then
...
function CountryEntity:match_get()
...
function CountryEntity:stream(action, args, callopts)
...
---@param reqmatch CountryLoadMatch
---@param ctrl? table
---@return Country
---@return string? err
function CountryEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
function CountryEntity:_run_op(ctx, post_done)
  local utility = self._utility
