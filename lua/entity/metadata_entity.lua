-- WorldBankData SDK Metadata entity

local vs = require("utility.struct.struct")
local helpers = require("core.helpers")

local MetadataEntity = {}
MetadataEntity.__index = MetadataEntity


function MetadataEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, MetadataEntity)
  self._name = "metadata"
  self._client = client
...
function MetadataEntity:get_name()
  return self._name
end


function MetadataEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return MetadataEntity.new(self._client, opts)
end
...
function MetadataEntity:mark_deleted()
  self._deleted = true
end


function MetadataEntity:deleted()
  return true == self._deleted
end


function MetadataEntity:data_set(args)
  if args ~= nil then
...
function MetadataEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function MetadataEntity:match_set(args)
  if args ~= nil then
...
function MetadataEntity:match_get()
...
function MetadataEntity:stream(action, args, callopts)
...
function MetadataEntity:_run_op(ctx, post_done)
  local utility = self._utility
