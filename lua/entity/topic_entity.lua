...
local TopicEntity = {}
TopicEntity.__index = TopicEntity


function TopicEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, TopicEntity)
  self._name = "topic"
  self._client = client
...
function TopicEntity:get_name()
  return self._name
end


function TopicEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return TopicEntity.new(self._client, opts)
end
...
function TopicEntity:mark_deleted()
  self._deleted = true
end


function TopicEntity:deleted()
  return true == self._deleted
end


function TopicEntity:data_set(args)
  if args ~= nil then
...
function TopicEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function TopicEntity:match_set(args)
  if args ~= nil then
...
function TopicEntity:match_get()
...
function TopicEntity:stream(action, args, callopts)
...
function TopicEntity:_run_op(ctx, post_done)
  local utility = self._utility
