...
local WorldBankDataSDK = {}
WorldBankDataSDK.__index = WorldBankDataSDK
...
WorldBankDataSDK._make_feature = _make_feature


function WorldBankDataSDK.new(options)
  local self = setmetatable({}, WorldBankDataSDK)
  self.mode = "live"
  self.features = {}
  self.options = nil
...
function WorldBankDataSDK:options_map()
...
function WorldBankDataSDK:get_utility()
  return Utility.copy(self._utility)
end


function WorldBankDataSDK:get_root_ctx()
  return self._rootctx
end


function WorldBankDataSDK:prepare(fetchargs)
...
function WorldBankDataSDK:direct(fetchargs)
...
-- by passing it.
function WorldBankDataSDK:_raw_request(fetchargs)
  local utility = self._utility
...
function WorldBankDataSDK:graphql(query, variables, ctrl)
  ...
  if type(errors) == "table" and 0 < #errors then
    ...
    res.ok = false
    res.err = "WorldBankDataSDK: graphql: " .. msg
    res.graphql = errors
  end
...
function WorldBankDataSDK.test(testopts, sdkopts)
  sdkopts = sdkopts or {}
  ...
  local sdk = WorldBankDataSDK.new(sdkopts)
  sdk.mode = "test"
