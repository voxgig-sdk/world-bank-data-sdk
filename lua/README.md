# WorldBankData Lua SDK

The Lua SDK for the WorldBankData API. Provides an entity-oriented interface using Lua conventions.
...
```lua
local result, err = client:Country(nil):list(nil, nil)
...
```lua
local result, err = client:Country(nil):load({ id = "example_id" }, nil)
if err then error(err) end
print(result)
```
...
local result, err = client:WorldBankData(nil):load(
  { id = "test01" }, nil
)
...
```
lua/
├── world-bank-data_sdk.lua    -- Main SDK module
...
The main module (`world-bank-data_sdk`) exports the SDK constructor
