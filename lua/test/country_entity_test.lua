...
describe("CountryEntity", function()
  ...
  it("should stream", function()
    ...
    local seen = {}
    for item in base:Country(nil):stream("list", nil, nil) do
    ...
    if type(config.feature) == "table" and config.feature.streaming ~= nil then
      ...
      local got = {}
      for item in streamsdk:Country(nil):stream("list", nil, nil) do
  ...
  it("should run basic flow", function()
    ...
    if setup.synthetic_only then
      pending("live entity test uses synthetic IDs from fixture — set WORLDBANKDATA_TEST_COUNTRY_ENTID JSON to run live")
      return
    end
    local client = setup.client

    -- Bootstrap entity data from existing test data.
    local country_ref01_data_raw = vs.items(helpers.to_map(
      vs.getpath(setup.data, "existing.country")))
    local country_ref01_data = nil
    if #country_ref01_data_raw > 0 then
      country_ref01_data = helpers.to_map(country_ref01_data_raw[1][2])
    end
    ...
    assert.is_nil(err)
    local country_ref01_data_dt0_load_result = helpers.to_map(country_ref01_data_dt0_loaded)
...
function country_basic_setup(extra)
  ...
  local entid_env_raw = os.getenv("WORLDBANKDATA_TEST_COUNTRY_ENTID")
