...
describe("CountryDirect", function()
  it("should direct-list-country", function()
    local setup = country_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-country", setup.live and "live" or "unit")
  ...
  it("should direct-load-country", function()
    local setup = country_direct_setup({ id = "direct01" })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-load-country", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
...
function country_direct_setup(mockres)
