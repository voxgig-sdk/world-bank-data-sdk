...
describe("IndicatorDirect", function()
  it("should direct-list-indicator", function()
    local setup = indicator_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-indicator", setup.live and "live" or "unit")
  ...
  it("should direct-load-indicator", function()
    local setup = indicator_direct_setup({ id = "direct01" })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-load-indicator", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
...
function indicator_direct_setup(mockres)
