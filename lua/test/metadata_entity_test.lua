...
describe("MetadataEntity", function()
  ...
  it("should stream", function()
    ...
    local seen = {}
    for item in base:Metadata(nil):stream("list", nil, nil) do
    ...
    if type(config.feature) == "table" and config.feature.streaming ~= nil then
      ...
      local got = {}
      for item in streamsdk:Metadata(nil):stream("list", nil, nil) do
  ...
  it("should run basic flow", function()
    ...
    if setup.synthetic_only then
      pending("live entity test uses synthetic IDs from fixture — set WORLD_BANK_DATA_TEST_METADATA_ENTID JSON to run live")
      return
    end
    local client = setup.client

    -- Bootstrap entity data from existing test data.
    local metadata_ref01_data_raw = vs.items(helpers.to_map(
      vs.getpath(setup.data, "existing.metadata")))
    local metadata_ref01_data = nil
    if #metadata_ref01_data_raw > 0 then
      metadata_ref01_data = helpers.to_map(metadata_ref01_data_raw[1][2])
    end
...
function metadata_basic_setup(extra)
  ...
  local entid_env_raw = os.getenv("WORLD_BANK_DATA_TEST_METADATA_ENTID")
